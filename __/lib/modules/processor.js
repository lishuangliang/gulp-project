    ...
    var _ref5 = _asyncToGenerator(function* (path, mainPath, absolute = false) {
        ...
        if (options.url instanceof Object) {
            for (let key in options.url) {
                ...
                if (/\$[01]$/.test(urlPattern) && (0, _minimatch2.default)(path, key, { matchBase: true })) {
                    _path = _path3.default.join('/', _path3.default.relative(options.cwd, info.rest));
                    if (/\$0$/.test(urlPattern)) _path = _path3.default.join(urlPattern.replace('$0', ''), _path);
                    if (/\$1$/.test(urlPattern)) _path = _path3.default.join(urlPattern.replace('$1', ''), _path3.default.parse(_path).base);
                    if (_path3.default.isAbsolute(_path)) {
                        path = _path3.default.join(options.cwd, _path);
                    } else {
                        path = _path;
                    }
                }
        ...
        if (_path3.default.isAbsolute(path)) {
            if (options.relative && !absolute) {
                path = _path3.default.relative(_path3.default.dirname(mainPath), info.rest);
            } else {
                path = _path || _path3.default.join('/', _path3.default.relative(options.cwd, info.rest)); //绝对地址
            }
        }

        let pathInfo = _path3.default.parse(path);
        if (_utils2.default.isResFile(path) && options.hash && path.indexOf('_' + md5 + '.') == -1) {
            path = _path3.default.join(pathInfo.dir, pathInfo.name + '_' + md5 + pathInfo.ext); //md5文件名字
        }
        path += info.query;
        if (_utils2.default.isResFile(path) && !options.hash && path.indexOf('_' + md5 + '.') == -1) {
    ...
    var _ref14 = _asyncToGenerator(function* (path, $) {
        ...
        yield eachHtml($, 'link[concat],script[concat]', (() => {
            var _ref15 = _asyncToGenerator(function* (el, tag) {
                ...
                if (_utils2.default.isRelativePath(src)) {
                    src = _path3.default.resolve(_path3.default.dirname(path), (yield uri(_utils2.default.inlinePath(path, src).absolute, path)));
...
function dev(contents) {
    ...
    return options.relative ? _path3.default.relative(_path3.default.dirname(mainPath), path) : _path3.default.resolve(options.cwd, path);
}