 ...
 * @type {{
     ...
     * release: null 发布地址转换,
     * uri: null 访问地址转换,
     * compression: boolean 是否压缩js和css
     * trigger: {
        ...
        function _(patterns) {
            ...
            if (patterns instanceof Object) {
                for (let key in patterns) {
                    ...
                    if (info.isLocalPath && process.platform != 'win32') pattern = utils.toAbsolute(pattern); //转换为物理路径
    ...
    extHTML(contents, file) {
        ...
        this.eachHtml($, 'link,script,a,iframe,img,embed,audio,video,object,source', (el, tag) => {
            ...
            for (let name of [attr, 'data-' + attr]) {
                ...
                let inlinePath = utils.inlinePath(file.path, url);
