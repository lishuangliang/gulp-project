...
module.exports = {
    ...
    parsePath(path) {
        if (!path) {
            ...
            info.isLocalPath = !!(info.rest && !/\/\?\?/.test(info.rest) && (pth.isAbsolute(info.rest) || info.protocol == null));
            return info;
        }
    },
    /**
     ...
     * @returns {*}
     */
    formatPath(path, query, hash) {
        if (query || hash) {
            path += (~path.indexOf('?') ? '&' : '?') + query + hash;
            let info = this.parsePath(path),
                map = {};
            info.query = info.query.split('&').filter((val) => {
                if (!map[val]) {
                    map[val] = true;
                    return true;
                } else {
            ...
            path = info.rest + '?' + info.query + info.hash;
        }
