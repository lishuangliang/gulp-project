...
class File {
    constructor(path) {
        ...
        if (process.platform == 'win32') {
            this.subpath = utils.parsePath(pth.resolve('/', pth.relative(process.cwd(), path))).path; //文件基于项目 root 的绝对路径
...
module.exports = function (path) {
    let info = utils.parsePath(path);
    path = (info.protocol || '') + info.pathname;
    if (!FILE_CACHE[path]) {
